...
public class ExperienceHandler {
	...
	public void onLevelUp(int level) {
		...
		if (level%75 == 0) {
			MaidHelper.giveItem(new ItemStack(Items.EMERALD, MathHelper.ceil(level/150f)), theMaid);
		}
		if (level%150 == 0) {
	...
	public boolean onDeath(DamageSource cause) {
		...
		if (theMaid.getMaidLevel() >= 20 && !cause.getDamageType().equals("outOfWorld") && !cause.getDamageType().equals("lmmnx_timeover") && !isWaitRevive) {
			...
			LittleMaidReengaged.Debug("DISABLING Remote=%s", theMaid.world.isRemote);
	...
	public void onUpdate() {
		if (!theMaid.world.isRemote && isWaitRevive) {
	...
	public boolean onDeathUpdate() {
		if (isWaitRevive && deathCount > 0) {
			return true;
		} else if (!theMaid.world.isRemote && theMaid.getHealth() <= 0f) {
	...
	public void readEntityFromNBT(NBTTagCompound tagCompound) {
		...
		if (causeType != null && !causeType.isEmpty()) {
			...
			if (causeEntityTag != null) {
				Entity entity = EntityList.createEntityFromNBT(causeEntityTag, theMaid.world);
				if (entity != null) {
