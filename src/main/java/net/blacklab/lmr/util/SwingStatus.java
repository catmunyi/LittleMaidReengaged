...
// アイテムの使用に関わる関数群
	...
	public void stopUsingItem(Entity pEntity) {
		if (!itemInUse.isEmpty() && pEntity instanceof EntityPlayer) {
			itemInUse.onPlayerStoppedUsing(pEntity.world, (EntityPlayer)pEntity, itemInUseCount);
		}
	...
	protected void updateItemUse(Entity pEntity, int par2) {
		if (itemInUse.getItemUseAction() == EnumAction.DRINK) {
			pEntity.playSound(SoundEvent.REGISTRY.getObject(new ResourceLocation("entity.generic.drink")), 0.5F, pEntity.world.rand.nextFloat() * 0.1F + 0.9F);
		}
		...
		if (itemInUse.getItemUseAction() == EnumAction.EAT) {
			...
			for (int var3 = 0; var3 < par2; ++var3) {
				...
				*/
				pEntity.world.spawnParticle(EnumParticleTypes.ITEM_CRACK, pEntity.posX, pEntity.posY, pEntity.posZ, (rand.nextFloat() - 0.5D) * 0.1D, Math.random() * 0.1D + 0.1D, 0.0D);
			}
	...
	protected void onItemUseFinish(EntityPlayer pEntityPlayer) {
		if (!this.itemInUse.isEmpty()) {
			...
			int var1 = this.itemInUse.getCount();

			// TODO いる？
			ItemStack var2 = itemInUse.onItemUseFinish(pEntityPlayer.world, pEntityPlayer);

			if (var2 != this.itemInUse || !var2.isEmpty() && var2.getCount() != var1) {
				if (var2.getCount() == 0) {
					pEntityPlayer.inventory.setInventorySlotContents(index, ItemStack.EMPTY);
				} else {
